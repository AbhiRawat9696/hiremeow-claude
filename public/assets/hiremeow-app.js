...
export function createHireMeowApp(React, components) {
 ...
 return function HireMeowApp(){
  ...
  const portfolioSlug=typeof location!=='undefined'?new URLSearchParams(location.search).get('portfolio'):null;
  if(portfolioSlug)return h('div',{className:'hiremeow'},h(P.PortfolioPage,{slug:portfolioSlug}));
  return h('div',{className:'hiremeow'},
   ...
   view==='lab'&&h('main',{id:'tool-section',className:'hm-dashboard'},h('div',{className:'hm-dashboard-heading'},h('p',{className:'hm-eyebrow'},'MEOW LAB · NEW'),h('h1',null,'Get hired, the fun way.'),h('p',{className:'hm-lab-sub'},'Roast your resume, practise interviews, earn badges and find the workplace that fits you.')),h(P.Studio,{platform,onOpenAuth:openAuth}),h(MeowLab)),
   ...
   view==='jobs'&&h('main',{id:'tool-section',className:'hm-dashboard'},h('div',{className:'hm-dashboard-heading'},h('p',{className:'hm-eyebrow'},'JOBS IN THAILAND'),h('h1',null,'Find a job with the salary up front.'),h('p',{className:'hm-lab-sub'},'Filter by BTS/MRT stop, distance and pay. Every listing shows a real salary range.')),platform.ready?(platform.live?h(P.JobsBoard,{platform,onOpenAuth:openAuth,onNavigate:v=>{open(v,false);window.scrollTo({top:0});}}):h(P.ui.SetupNeeded,{what:'the Jobs board'})):h('p',null,'Loading…')),
